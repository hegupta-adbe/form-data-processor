...
export class Workbook extends NamedItemContainer {
  ...
  async createSession(persistent = true) {
    ...
    if (sessionId) {
      if (this._oneDrive.workbookSessionPersistent === persistent) {
        return sessionId;
      } else {
        throw new StatusCodeError('Session exists but with different persistence than what is required!', 400);
      }
    } else {
      ...
      this._oneDrive.setWorkbookSessionPersistent(persistent);
      return result.id;
    }
  }
  ...
  async closeSession() {
    if (this._oneDrive.workbookSessionId) {
      ...
      // Reset to the reasonable default
      this._oneDrive.setWorkbookSessionPersistent(true);
    } else {
  ...
  setSessionId(sessionId, persistent = true) {
